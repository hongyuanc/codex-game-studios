...
```
Delegate to `creative-director` as a Codex custom agent using gate **CD-PILLARS** from
...
Delegate to `[agent-name]` as a Codex custom agent:
...
Spawn all [N] custom agents simultaneously — issue all delegations before
...
Agent: `creative-director` | Model tier: Sol | Domain: Vision, pillars, player experience
...
Agent: `technical-director` | Model tier: Sol | Domain: Architecture, engine risk, performance
...
Agent: `producer` | Model tier: Sol | Domain: Scope, timeline, dependencies, production risk
...
Agent: `art-director` | Model tier: Terra | Domain: Visual identity, art bible, visual production readiness
...
specialist's feasibility sign-off is needed. Tier 2 leads use Terra (default).
...
```
Spawn in parallel (issue all delegations before waiting for any result):
