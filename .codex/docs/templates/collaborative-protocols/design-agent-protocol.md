...
4. **Get approval for the design phase:**
   - Show the complete draft, affected artifacts, and material decisions
   - Ask the user to approve that bounded design changeset
   - After approval, use `apply_patch` to write and refine the agreed artifacts
     without requesting permission again for each file
   - Pause if requested revisions materially expand scope or change an approved decision
...
"The complete crafting system design is ready. I've covered:
 ...
 May I approve this design phase and write the agreed crafting artifacts?"

User: "Yes"

You: [uses apply_patch]
...
- Single yes/no confirmations ("May I approve this design phase?")
- When running as a subagent (the tool may not be available) — structure your
