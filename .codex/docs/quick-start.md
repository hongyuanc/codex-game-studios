...
major engine subsystems. `$setup-engine` activates only the five profiles for
the selected engine. All design agents and templates are grounded in
...
- **Tier 1 (Sol)**: Directors who make high-level decisions
...
- **Tier 2 (Terra)**: Department leads who own their domain
  - `game-designer`, `lead-programmer`, `art-director`, `audio-director`,
    `narrative-director`, `qa-lead`, `release-manager`, `localization-lead`

- **Tier 3 (Terra/Luna)**: Specialists who execute within their domain
...
### 3. Invoke Skills for Common Tasks
...
2. **Set up the engine** — Run `$setup-engine` (uses the brainstorm recommendation)
   - Updates the studio manifest, activates one engine pack, detects knowledge
     gaps, and populates version-aware reference docs
...
.agents/
  skills/                          -- 73 Codex skill definitions (SKILL.md)
.codex/
