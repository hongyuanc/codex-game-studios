---
name: test-helpers
description: "Use when repeated test setup, assertions, factories, or engine-specific fixtures are creating boilerplate across test files."
---
...
**When to run:**
- After `$codex-game-studios:test-setup` scaffolds the framework (first time)
...
**Modes:**
- `$codex-game-studios:test-helpers [system-name]` — generate helpers for a specific system
  (e.g., `$codex-game-studios:test-helpers combat`)
- `$codex-game-studios:test-helpers all` — generate helpers for all systems with test files
- `$codex-game-studios:test-helpers scaffold` — generate only the base helper library (no
...
If engine is not configured: "Engine not configured. Run `$codex-game-studios:setup-engine` first."
...
```
Search for `tests/**/*_test.*` (all test files).
```
...
## Generated by $codex-game-studios:test-helpers combat on [date].
...
Present a complete summary of every new or modified file that will be created:
...
Ask once: "May I write this complete helper changeset?"

After approval, create only the listed paths; do not ask again for each listed file. Pause for a revised preflight if any additional file or behavior becomes necessary.
...
- Run `$codex-game-studios:test-setup` if the test framework has not been scaffolded yet.
- Use `$codex-game-studios:dev-story` to implement stories — helpers reduce boilerplate in new test files.
- Run `$codex-game-studios:skill-test` to validate other skills that may need helper coverage.
