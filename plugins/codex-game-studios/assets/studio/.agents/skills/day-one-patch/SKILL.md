...
Delegate to `release-manager` through Codex custom-agent delegation. Ask them to produce a rollback plan covering:
...
Present the rollback plan and keep it as an in-memory draft. Do not proceed to Phase 4 until the parent has shown one complete implementation preflight containing the rollback plan, patch record, every source/config edit, every test/evidence file, exact acceptance criteria, and rollback risk, and the user has approved that complete proposed changeset.

---

## Phase 4: Implement Fixes

For each bug in the approved scope, delegate to a focused implementation loop:

1. Delegate to `lead-programmer` through Codex custom-agent delegation with:
...
3. Delegate to `qa-tester` through Codex custom-agent delegation to verify: does the bug reproduce after the fix?
...
The approved complete proposed changeset must include both `production/releases/rollback-plan-[version].md` and `production/releases/day-one-patch-[version].md`. Write only the files and edits listed in that approval.
