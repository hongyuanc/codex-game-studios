...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and ask one concise question and wait for the answer:
...
Present the full proposed inventory to the user in conversation. Then ask one concise question and wait for the answer:
...
The user's "Looks good — save this inventory" approval authorizes writing `design/assets/entity-inventory.md`; do not ask a second write question.
...
**Mode behavior:**
- `full` (default): delegate to both `art-director` and `technical-artist` in parallel
- `lean`: delegate to `art-director` only — faster, skips technical constraint pass
- `solo`: no agent delegation — main session writes specs from art bible rules alone. Use for simple asset categories or when speed matters more than depth.
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, ask one concise question and wait for the answer:
...
- **Related specs**: repository file search `design/assets/specs/*.md` — scan for assets that could be shared (e.g., a common UI element specced for one system might apply here too).
...
Present the full identified list to the user. Ask one concise question and wait for the answer:
...
Delegate to specialist agents based on review mode. **Issue all Codex custom-agent delegations simultaneously — do not wait for one before starting the next.**

### Full mode — delegate in parallel to these named Codex custom-agent roles:

**`art-director`** through Codex custom-agent delegation:
...
**`technical-artist`** through Codex custom-agent delegation:
...
### Lean mode — delegate to art-director only (skip technical-artist).
...
After presenting all specs, ask one concise question and wait for the answer:
...
If [B]: ask which asset and what to change. Revise inline and re-present. Do NOT delegate to agents again for minor text revisions — only delegate again if the visual direction itself needs to change.

If [C]: ask what direction to change. Delegate again to the relevant agent with the updated brief.

---

## Phase 5: Write Approved Changeset

Before the Phase 4 approval question, list both `design/assets/specs/[target-name]-assets.md` and `design/assets/asset-manifest.md` with their proposed changes. The "Approve all — write to file" choice authorizes writing that full changeset; do not ask again per file.
...
The Phase 4 full-changeset approval also authorizes this manifest update.
...
```
repository text search pattern="ASSET-" path="design/assets/asset-manifest.md"
```
...
If any consulted agent returns BLOCKED or cannot complete:
