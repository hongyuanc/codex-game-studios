---
name: balance-check
description: Analyze game balance data and formulas for outliers, broken progression, degenerate strategies, and economy risks.
---

## Codex-native operating rules

Keep repository source artifacts read-only during analysis and cite concrete file evidence for every finding. Ask at most one user question per turn and wait for the answer. A report may be written only after its exact path and contents are approved. Do not fix reviewed source artifacts unless the user gives separate explicit authorization.
...
After presenting the report, ask one concise question and wait for the answer:
...
- If the fix changes a tuning knob defined in a GDD or referenced by an ADR, remind the user:
  > "This value is defined in a design document. Run `$codex-game-studios:propagate-design-change [path]` on the affected GDD to find downstream impacts before committing."
...
- Confirm the file was written, then end with: "Re-run `$codex-game-studios:balance-check` after fixes to verify."

If [C]:
- Summarize open issues and end with: "Re-run `$codex-game-studios:balance-check` after fixes to verify."
