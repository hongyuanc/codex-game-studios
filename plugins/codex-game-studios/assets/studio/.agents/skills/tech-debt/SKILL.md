---
name: tech-debt
description: Scan, record, prioritize, or report technical debt in the game project.
---

## Codex-native operating rules

Keep repository source artifacts read-only during analysis and cite concrete file evidence for every finding. Ask at most one user question per turn and wait for the answer. The debt register may be written only after its exact changeset and path are approved. Do not fix source debt unless the user gives separate explicit authorization.
...
Then ask one concise question and wait for the answer to collect the **category**:
...
Then ask one concise question and wait for the answer to collect the **estimated fix effort**:
