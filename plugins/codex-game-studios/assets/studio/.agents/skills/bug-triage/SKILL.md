---
name: bug-triage
description: "Use when open game bugs need priority and severity review, sprint assignment, or systemic trend analysis."
---
...
- After `$codex-game-studios:team-qa` completes and new bugs have been filed
...
**Modes:**
- `$codex-game-studios:bug-triage sprint` — triage against the current sprint; assign fixable bugs
  to the sprint backlog; defer the rest
- `$codex-game-studios:bug-triage full` — full triage of all bugs regardless of sprint scope
- `$codex-game-studios:bug-triage trend` — trend analysis only (no assignment); read-only report
...
> **Generated by**: $codex-game-studios:bug-triage
...
- If any S1 bugs are unassigned: "S1 bugs must be assigned before the sprint
  can be considered healthy. Run `$codex-game-studios:sprint-status` to see current capacity."
- If regression bugs exist: "Regressions found — consider re-opening the
  affected stories in sprint tracking and running `$codex-game-studios:smoke-check` to re-gate."
