---
name: scope-check
description: Compare a feature or sprint with its approved scope and report scope creep and recommended cuts.
---
...
- **CONCERNS** → offer to identify the 2–3 additions with best cut ratio. Reference `$codex-game-studios:sprint-plan update` to formally re-scope.
- **FAIL** → recommend escalating to producer. Reference `$codex-game-studios:sprint-plan update` for re-planning or `$codex-game-studios:estimate` to re-baseline timeline.

Always end with:
> "Run `$codex-game-studios:scope-check [name]` again after cuts are made to verify the verdict improves."
