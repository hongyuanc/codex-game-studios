...
1. **Parse the argument** for an optional genre/theme hint (e.g., `roguelike`,
   ...
   scratch. Also resolve the review mode (once, store for all gate delegations this run):
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Ask one concise question and wait for the answer** at key decision points throughout brainstorming:
   ...
   a one-question prompt to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in a one-question prompt with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Ask one concise question and wait for the answer for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Ask these as separate concise questions, one per turn, waiting for each answer. Preserve these labels:
...
Present all three. Then ask one concise question and wait for the answer to capture the selection.

**CRITICAL**: Ask one concept-selection question and wait for the answer. Offer Concept 1, Concept 2, Concept 3, Combine elements across concepts, and Generate fresh directions as the documented choices, each with a concise description. Do not ask another question in the same turn.
...
Ask these as separate one-question prompts, one per turn, waiting for each answer — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, ask one concise question and wait for the answer:
...
If the user selects B, C, or D, make the revision, then ask one concise question and wait for the answer again:
...
**Review mode check** — apply before delegating to CD-PILLARS and AD-CONCEPT-VISUAL:
...
**Review mode check** — apply before delegating to TD-FEASIBILITY:
...
5. Ask one concise question and wait for the answer for write approval:
...
If [B]: ask which section to revise using one concise question, then wait for the answer with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then ask one concise question and wait for the answer — "Ready to write the updated concept document?"
