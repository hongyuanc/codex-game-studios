...
**Dependency graph validation:** For every system listed in the Dependencies section, search the repository to check whether its GDD file exists in `design/gdd/`. Flag any that don't exist yet — these are broken references that downstream authors will hit.
...
**Before delegating to any agents**, print this notice:
> "Full review: delegating to specialist agents in parallel. This typically takes 8–15 minutes. Use `--review lean` for faster single-session analysis."
...
| If the GDD contains... | Delegate to these agents |
|------------------------|-------------------|
...
Delegate to `game-designer` for all GDDs that describe gameplay mechanics or player-facing rules.
Delegate to `systems-designer` for all GDDs that contain formulas or system interaction rules.
...
### Step 2 — Delegate to all relevant specialists in parallel

**CRITICAL: Delegation in this skill must target the named Codex custom-agent roles. Each role receives its own bounded context. Do not simulate specialist perspectives and claim that roles were consulted; a simulated review is not a specialist review.**

Issue all Codex custom-agent delegations simultaneously. Start independent delegations together; do not wait for one before starting another.
...
After all specialists respond, delegate to `creative-director` as the **senior reviewer**:
...
Specialists consulted: [list agents consulted]
...
Ask one concise question and wait for the answer for ALL closing interactions. Never plain text.

**First prompt — what to do next:**

If APPROVED (first-pass, no revision needed), proceed directly to the systems-index prompt, review-log prompt, then the final closing prompt. Do not show a separate "what to do" prompt — the final closing prompt covers next steps.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Ask each required design-decision question in its own turn and wait for the answer. After the decisions are complete, present the exact proposed fix changeset and paths. Do not edit the GDD until the user separately authorizes that changeset.

After all revisions are complete, show a summary table (blocker → fix applied) and ask one concise question and wait for the answer for a **post-revision closing prompt**:
...
**Final closing prompt — always show after all file writes complete:**

Once the systems-index and review-log prompts are answered, check project state and show one final one-question prompt:
...
Never end the skill with plain text after file writes. Always close with this prompt.
