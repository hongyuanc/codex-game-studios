...
Run git log for the sprint period to understand what was actually committed and when. Use the shell (`2>/dev/null` is bash syntax, not PowerShell):

```
```bash
git log --oneline --since="4 weeks ago" 2>/dev/null || git log --oneline -20
```
```
