...
1. repository search `tests/unit/[system]/` and `tests/integration/[system]/` for file names
...
2. repository search `tests/unit/[system]/` and `tests/integration/[system]/` for a test
...
- GDD sections added or revised since the regression suite was last updated
  (use repository search on GDD file modification hints if available, or ask the user)
...
Before asking, show the exact proposed entries to add, retain, revise, or remove, with evidence paths and reasons. Then ask for approval of the complete `tests/regression-suite.md` changeset. Do not update the registry without that approval.

For `update` mode: append approved new entries; never remove existing entries.
