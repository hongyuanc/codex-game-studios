...
description: Create or revise an Architecture Decision Record for a significant technical choice.
---
...
(e.g., `$codex-game-studios:architecture-decision retrofit docs/architecture/adr-0001-event-system.md`):

Enter **retrofit mode**:

1. Read the existing ADR file completely.
2. Identify which template sections are present by scanning headings:
   - `## Status` — **BLOCKING if missing**: `$codex-game-studios:story-readiness` cannot check ADR acceptance
...
5. If yes:
   ...
   - Append each missing section to the ADR file using the targeted patch.
...
7. Suggest: "Run `$codex-game-studios:architecture-review` to re-validate coverage now that this ADR
...
6. **Display a knowledge gap warning** before proceeding if the domain carries
   ...
   Run `$codex-game-studios:setup-engine` first, or tell me which engine you are using."
...
a **confirm/adjust** prompt using one concise question, then wait for the answer — not open-ended questions.
...
**Scope of assumptions question**: Assumptions cover only problem framing, alternative approaches, upstream dependencies, GDD linkage, and status. Schema design questions (e.g., "How should spawn timing work?", "Should data be inline or external?") are NOT assumptions; ask them separately after the assumptions are confirmed.

**After assumptions are confirmed**, if the ADR involves schema or data design choices, ask each design question separately, one per turn, and wait for each answer before drafting.

**Present assumptions with one concise question, then wait for the answer:**
...
decisions remain, present each one as a separate one-question prompt with the proposed
...
| **References Consulted** | [List stable source labels, e.g. `Codex Game Studios bundled engine reference: godot/modules/physics.md`] |
...
5.5. **Engine Specialist Validation** — Before saving, delegate to the **primary engine specialist** through Codex custom-agent delegation to validate the drafted ADR:
   ...
   - Delegate to `[primary specialist] Codex custom-agent role` with: the ADR's Engine Compatibility section, Decision section, Key Interfaces, and the engine reference docs path. Ask them to:
...
**Registry append logic**: When writing to `docs/registry/architecture.yaml`, do NOT assume sections are empty. The file may already have entries from previous ADRs written in this session. Before each targeted patch:
...
Ask using one concise question, then wait for the answer:
...
After the ADR is written (and registry optionally updated), close with one concise question, then wait for the answer.
...
[N] Start writing GDDs — run `$codex-game-studios:design-system [first-undesigned-system]` (only show if all prerequisite ADRs are written)
[N+1] Stop here for this session
```

If there are no remaining priority ADRs and no undesigned GDD systems, offer only "Stop here" and suggest running `$codex-game-studios:architecture-review` in a fresh session.
...
> and run `$codex-game-studios:architecture-review`.
>
> **Never run `$codex-game-studios:architecture-review` in the same session as `$codex-game-studios:architecture-decision`.**
