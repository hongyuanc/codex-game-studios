---
name: perf-profile
description: Profile a game system or build against performance budgets and prioritize observed bottlenecks.
---

## Codex-native operating rules

Keep repository source artifacts read-only during analysis and cite concrete file evidence for every finding. Ask at most one user question per turn and wait for the answer. A report may be written only after its exact path and contents are approved. Do not apply optimizations unless the user gives separate explicit authorization.
