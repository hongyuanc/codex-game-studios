...
class ReleaseContractTests(unittest.TestCase):
    ...
    def test_documented_marketplace_refs_use_published_main_branch(self):
        # Arrange
        ...
        documented_refs = [
            ...
            for match in re.finditer(
                r"codex plugin marketplace add hongyuanc/codex-game-studios --ref (\S+)",
        ...
        for case in cases:
            with self.subTest(case=case), tempfile.TemporaryDirectory() as directory:
                ...
                if case == "plugins ancestor":
                    ...
                    output = root / "dist"
                    output.mkdir()
                    stale_archive = output / "codex-game-studios-1.0.1.zip"
    ...
    def assert_regular_swap_rejected(self, target_relative, attacker_bytes):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            output = root / "dist"
            archive = output / "codex-game-studios-1.0.1.zip"
    ...
    def test_release_packager_early_failure_cleans_only_plugin_outputs(self):
        ...
        for case in cases:
            with self.subTest(case=case), tempfile.TemporaryDirectory() as directory:
                ...
                output.mkdir()
                stale = (
                    output / "codex-game-studios-1.0.1.zip",
                    output / "codex-game-studios-1.0.1.zip.sha256",
                    output / ".codex-game-studios-1.0.1.zip.deadbeef.tmp",
                    output / ".codex-game-studios-1.0.1.zip.sha256.deadbeef.tmp",
                )
                for path in stale:
                ...
                unrelated = {
                    ...
                    output / "codex-game-studios-1.0.1.zip.backup": b"backup",
                }
    ...
    def test_release_packager_cleanup_unlinks_links_and_specials_without_following(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            archive = output / "codex-game-studios-1.0.1.zip"
            checksum = output / "codex-game-studios-1.0.1.zip.sha256"
    ...
    def test_release_finalization_mocked_windows_branch_verifies_installed_bytes(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            temporary = output / ".codex-game-studios-1.0.1.zip.deadbeef.tmp"
            final = output / "codex-game-studios-1.0.1.zip"
            ...
            def create_then_substitute(destination, label, *args, **kwargs):
                ...
                if is_target and not substituted:
                    ...
                    victims = {
                        ...
                        output / "codex-game-studios-1.0.1.zip": b"attacker zip victim",
                        output
                        / "codex-game-studios-1.0.1.zip.sha256": b"attacker checksum victim",
                    }
    ...
    def test_release_packager_cli_prints_archive_and_checksum_paths(self):
        ...
        self.assertEqual(
            [
                str(output.resolve() / "codex-game-studios-1.0.1.zip"),
                str(output.resolve() / "codex-game-studios-1.0.1.zip.sha256"),
            ],
            completed.stdout.splitlines(),
        )
