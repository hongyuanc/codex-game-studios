...
**Retrofit mode detection**: repository file search `design/art/art-bible.md`. If the file exists:
- Read it in full
...
Ask one concise question and wait for the answer as two separate questions, one per turn, waiting for each answer:
...
**Agent delegation (MANDATORY)**: Delegate to `art-director` through Codex custom-agent delegation:
...
Present the art-director's draft to the user. Ask one concise question and wait for the answer:
...
**Agent delegation**: Delegate in parallel to these named Codex custom-agent roles:
...
Collect both. If they conflict (e.g., art-director wants elaborate diegetic UI but ux-designer flags it would reduce combat readability), surface the conflict explicitly with both positions. Do NOT silently resolve — ask one concise question and wait for the answer to let the user decide.
...
- Never draft a section without first delegating to the relevant agent(s)
