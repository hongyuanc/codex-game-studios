...
For Unreal projects: automation logs go to `Saved/Logs/`. repository search for
...
- repository search for `<testcase name=` to get test names
- repository search for `<failure` or `<error` to identify failures
- Parse `classname` and `name` attributes for full test identifiers

**Plain text logs**:
- repository search for pass/fail patterns:
...
Use repository search to check the test file for timing calls, randf, global state access,
...
If yes: append the approved entries to the Quarantined Tests table.
