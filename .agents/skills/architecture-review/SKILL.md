...
```
repository text search pattern="## Summary" glob="design/gdd/*.md" output_mode="content" -A 4
repository text search pattern="## Summary" glob="docs/architecture/adr-*.md" output_mode="content" -A 3
```
...
repository file search `production/epics/**/*.md` (excluding EPIC.md index files). For each
story file:
...
repository file search `tests/unit/**/*_test.*` and `tests/integration/**/*_test.*`.
Build an index: system → [test file paths].

For each test file path from Step 3b-1, confirm via repository file search whether the file
...
After completing the engine audit above, delegate to the **primary engine specialist** through Codex custom-agent delegation for a domain-expert second opinion:
...
- Delegate to `[primary specialist] Codex custom-agent role` with: all ADRs that contain engine-specific decisions or `Post-Cutoff APIs Used` fields, the engine reference docs, and the Phase 5 audit findings. Ask them to:
