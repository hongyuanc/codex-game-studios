...
Otherwise, ask one concise question and wait for the answer to confirm intent before proceeding:
...
Assess which path best fits the hypothesis, then ask one concise question and wait for the answer with your
...
- `full` → delegate to `creative-director` through Codex custom-agent delegation using gate **CD-PLAYTEST** if
...
1. **Define the spike question** (plain text, not a prompt): "What specific question does this spike answer? Give me one sentence: 'Can we [do X] using [approach Y]?'"

2. **Choose path** — same one-question prompt as Phase 3 (HTML / Engine / Paper).
