...
Spawn all applicable specialists simultaneously through Codex custom-agent delegation — do not wait for one before starting the next.
...
For Logic and Integration stories, also spawn `qa-tester` through Codex custom-agent delegation in parallel with the engine specialists. Pass:
...
This skill is read-only — no files are written and no reviewed source is changed. It may recommend fixes, but implementation requires a separately authorized fix changeset.
