...
## Phase 2: Delegate to Security Engineer

Delegate to `security-engineer` through Codex custom-agent delegation. Pass:
...
Search patterns: `File.open`, `load`, `deserialize`, `JSON.parse`, `from_json`, `read_file` — check each for validation.
...
Search for: `recv`, `receive`, `PacketPeer`, `socket`, `NetworkedMultiplayerPeer`, `rpc`, `rpc_id` — check each call site for validation.
...
Search for: `get_input`, `Input.get_`, `input_map`, user-facing text fields — check validation.
...
Search for: `api_key`, `secret`, `password`, `token`, `private_key`, `DEBUG`, `print(` in release-facing code.
