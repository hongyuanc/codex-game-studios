...
- **No argument**: Auto-detect current stage using the same heuristics as
  ...
  - Options:
    - `[A] Yes — run this gate`
    - `[B] No — pick a different gate` (if selected, show a second prompt listing all gate options: Concept → Systems Design, Systems Design → Technical Setup, Technical Setup → Pre-Production, Pre-Production → Production, Production → Polish, Polish → Release)
...
### Artifact Checks
- Use `repository file search` and `Read` to verify files exist and have meaningful content
...
> **Evidence-action requirement**: At least 2 of the 5 challenge questions below must be answered by re-reading a specific file (repository read) or re-running a specific check (repository search) — not by reflection alone. Mark these with [TOOL ACTION] to indicate a tool was used.
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using one concise question, then wait for the answer.
