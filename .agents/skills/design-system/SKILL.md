...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and ask one concise question and wait for the answer:
...
6. If yes: proceed to **Phase 2 (Gather Context)** as normal, but in **Phase 3**
   ...
   incomplete sections.
7. **Never overwrite existing section content.** Use targeted patch to replace only
...
- **Related GDDs**: repository file search `design/gdd/*.md` and read any that are thematically related
...
- Search the repository to check if the file exists.
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   a one-question prompt for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then ask one concise question and wait for the answer to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   a one-question prompt. **NEVER use plain text. NEVER skip this step.**
   ...
   **The draft and the approval prompt MUST appear together in one response.
   If the draft appears without the prompt, the user is left at a blank prompt
   with no path forward — this is a protocol violation.**

7. **Write**: Use the targeted patch to replace the placeholder with the approved content.
   **CRITICAL**: Always include the section heading in the `old_string` to ensure
   ...
   same placeholder and the targeted patch requires a unique match. Use this pattern:
   ```
...
completed section name. Search the repository to check if the file exists, then create it if absent or apply a targeted patch if present.
...
**Derive recommended options before asking**: Read the system's category and layer from the systems index (already in context from Phase 2), then determine the recommended choice for each question:
- **Framing**: Foundation/Infrastructure layer → `[A]` recommended. Player-facing categories (Combat, UI, Dialogue, Character, Animation, Visual Effects, Audio) → `[C] Both` recommended.
- **ADR reference**: Search for files matching `docs/architecture/adr-*.md` and search for the system name in the GDD Requirements section of any ADR. If a matching ADR is found → `[A] Yes — cite the ADR` recommended. If none found → `[B] No` recommended.
- **Fantasy**: Foundation/Infrastructure layer → `[B] No` recommended. All other categories → `[A] Yes` recommended.

Append `(Recommended)` to the appropriate option for each question.

**Framing questions (ask BEFORE drafting)**: Ask these separately, one per turn, and wait for each answer:
...
**Derive recommended option before building the prompt**: Read the system's category and layer from Phase 2 context:
...
**Framing question (ask BEFORE drafting)**: Ask one concise question and wait for the answer:
...
**Review mode check** (apply before delegating to):
- `solo` → skip this agent delegation. Draft the section without the specialist. Add a note: "`creative-director` not consulted — Solo mode. Review manually before production."
...
- `full` → delegate to as described below.

**Agent delegation (MANDATORY)**: After the framing answer is given but before drafting,
delegate to `creative-director` through Codex custom-agent delegation:
...
**Review mode check** (apply before delegating to):
- `solo` → skip this agent delegation. Draft the section without the specialist. Add a note: "Specialist agents not consulted — Solo mode. Review manually before production."
- `lean` → skip unless this is a section with HIGH implementation risk (Sections D and H only). For other sections, draft without the agent.
- `full` → delegate to as described below.

**Agent delegation (MANDATORY)**: Before drafting Section C, delegate to specialist agents through Codex custom-agent delegation in parallel:
- Look up the system category in the routing table (Section 6 of this skill)
- Delegate to the Primary Agent AND Supporting Agent(s) listed for this category
...
- Surface any disagreements between agents to the user with one concise question, then wait for the answer
...
**Review mode check** (apply before delegating to):
- `solo` → skip this agent delegation. Draft the section without the specialist. Add a note: "`systems-designer` not consulted — Solo mode. Review manually before production."
- `lean` → skip unless this is a section with HIGH implementation risk (Sections D and H only). For other sections, draft without the agent.
- `full` → delegate to as described below.

**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, delegate to specialist agents through Codex custom-agent delegation in parallel:
- **Always delegate to `systems-designer`**: provide Core Rules from Section C, tuning goals from user, balance context from dependency GDDs. Ask them to propose formulas with variable tables and output ranges.
- **For economy/cost systems, also delegate to `economy-designer`**: provide placement costs, upgrade cost intent, and progression goals. Ask them to validate cost curves and ratios.
- Present the specialists' proposals to the user for review with one concise question, then wait for the answer
...
**Agent delegation (MANDATORY)**: Delegate to `systems-designer` through Codex custom-agent delegation before finalising edge cases. Provide: the completed Sections C and D, and ask them to identify edge cases from the formula and rule space that the main session may have missed. For narrative systems, also delegate to `narrative-director`. Present their findings and ask the user which to include.
...
```
repository text search pattern="  - name: [candidate_name]" path="design/registry/entities.yaml"
```
...
**When delegating through a named Codex custom-agent role**:
...
- The main session presents the agent's output to the user with one concise question, then wait for the answer
- The user decides; the main session writes to file
...
3. **Approval authorizes the listed write** — ask once before skeleton creation; after that, each section's explicit approval authorizes its write to the already identified GDD path
...
This is a long-running skill. After writing each section, if Codex reports
context usage at or above 70%, append this notice to the response:
